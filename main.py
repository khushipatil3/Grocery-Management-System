...
__copyright__ = "SRM University of Science and Technology "
...
class mainWindow():
    def __init__(self, window1):
        ...
        text_aboutus1 = "SRM University of Science and Technology."
        text_aboutus2 = "APP Project on SRM General Stores Management System"
        ...
        contact_menu.add_command(label="kp6870@srmist.edu.in")
        contact_menu.add_separator()
        contact_menu.add_command(label="vv@1234@srmist.edu.in")
        contact_menu.add_separator()
        ...
        help_menu2.add_command(label="kp6870@srmist.edu.in")
        help_menu.add_command(label="Exit", command = root.destroy)


        # self.back_img = Image.open('image.png')
        # self.back_img = self.back_img.resize((1500, 1000), Image.LANCZOS)
        ...
        self.heading = Label(self.window, text="SRM General Stores", font=('arial 60 bold'),image = self.logo1,compound = "left", fg='black', bg="white")
...
def mainFun():
    ...
    if password == actual:
        login.withdraw()
        root = Toplevel()
        root.title("SRM General Stores Management")
        mainWindow(root)
...
conn = mysql.connector.connect(host="localhost", user="root", password="khushimysql", database="grocerystore")
